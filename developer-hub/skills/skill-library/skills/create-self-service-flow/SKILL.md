...
- `tibco-examples/self-service-flows/build-deploy-flogo-app/self-service-build-deploy-flogo-app.yaml`
...
- `tibco-examples/self-service-flows/build-deploy-bw5ce-app/self-service-build-deploy-bw5ce-app.yaml`
  — the same pattern for a BW5CE `.ear` upload
- `tibco-examples/self-service-flows/tibco-self-service-group.yaml` — the owner group

If those paths do not exist in the checkout, get the **Build & Deploy Flogo App** entry from the
Marketplace first, or read the flows in the marketplace content repo under
`marketplace-content/developer-hub/self-service-flows/`.
...
fail. If it is not in the catalog, create `templates/<slug>/tibco-self-service-group.yaml`
(`kind: Group`, `spec.type: team`) and register it in `catalog.locations` too.
