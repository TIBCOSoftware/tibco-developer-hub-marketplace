...
- **Run it with the bundled Node**: `./node/bin/node .claude/skills/api-version-diff/apidiff.mjs …`
  (or plain `node` if you have one on PATH). No install step.
- **JSON specs work out of the box. YAML needs one conversion step** — unlike a Yarn monorepo, the
  portable bundle's `node_modules` is tree-shaken to what the backend loads at runtime, so it ships
  **no `yaml` or `js-yaml` package** and the helper will tell you so. The bundle's Python venv has
  PyYAML, so convert first:

  ```sh
  .venv/bin/python3 -c "import yaml,json,sys; json.dump(yaml.safe_load(open(sys.argv[1])), open(sys.argv[2],'w'))" \
    spec.yaml ${TMPDIR:-/tmp}/devhub-skills/api-version-diff/spec.json
  ```

  The TIBCO Platform capability specs are already `.json`, so this only bites on the Backstage
  catalog spec and hand-written YAML.
...
- **Two catalog entities** — read `spec.definition` from the running portable hub. Guest mode needs
  a bearer token here (see `.claude/skills/impact-analysis/SKILL.md` for how to mint one):
  ```sh
  ...
  curl -s -H "Authorization: Bearer $TOKEN" "$HUB/api/catalog/entities/by-name/api/default/<name>" \
    | python3 -c "import json,sys; print(json.load(sys.stdin)['spec']['definition'])" > ${TMPDIR:-/tmp}/devhub-skills/api-version-diff/old.yaml
  ```
  All localhost curls need `dangerouslyDisableSandbox: true`.
