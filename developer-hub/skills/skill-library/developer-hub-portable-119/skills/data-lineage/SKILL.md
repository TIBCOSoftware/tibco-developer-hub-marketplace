...
- **Auth**: guest mode is **not** anonymous here. Every `/api/*` call needs a bearer token or it
  ...
  curl -s -H "Authorization: Bearer $TOKEN" "$HUB/api/catalog/entities?filter=kind=api"
  ```
...
- **Endpoints**: `GET /entities?filter=…` for a filtered scan (with a `fields` projection),
  `POST /entities/by-refs` for batch fetches, `GET /entities/by-name/{kind}/{ns}/{name}` for one
  entity in full. Identical access rules to `.claude/skills/reuse-or-build/SKILL.md`. State the
  endpoints used in the report's provenance section.
- **`lineage.py` runs on the bundled Python.** It imports only the standard library (`json`, `re`,
  `sys`, `collections`), so `.venv/bin/python3 .claude/skills/data-lineage/lineage.py` works with no
  installs.
