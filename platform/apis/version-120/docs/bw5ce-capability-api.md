...
No endpoint changes this release — all 53 operations are unchanged. The only change is per-container detail on instance status, which lands identically in the BW6 and Flogo capability APIs. Unlike BW6, BW5 CE does **not** get the OSGi console endpoint.
...
Endpoint count stays at 53; schema count rises from 65 to 66.
...
`mode` is required and accepts `tag`, `untag`, `cleanup` or `bulk`. The first three are age-based and
require `ageInDays`: `tag` marks unused builds older than that age with `olderthan**age**days`,
`untag` removes that tag again, and `cleanup` deletes everything carrying it. `bulk` instead deletes
the specific builds listed in the repeatable `buildIds` query parameter and skips any build still in
use by an app. Removed builds cannot be recovered.
...
- **EndpointInfo**: added `publicSwaggerURL` (string) and `publicSwaggerJsonURL` (string), exposing the public Swagger UI and Swagger JSON endpoints of a deployed application alongside the existing `publicEndpoint` and `publicWsdlURLs`.

Endpoint count rises from 52 to 53; schema count from 64 to 65.
